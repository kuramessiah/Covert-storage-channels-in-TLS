...
psk = None
parser = ArgumentParser(description='TLS Client')
...
parser.add_argument("--ccm", type=str, default='', required=False,
                    dest='ccm', help="Covert message for sid-cc")
...
ans = input("Import a covert channel? [y/n]: ")
length = 32
...
if args.ciphersuite:
    ...
    ch = TLS13ClientHello(ciphers=0x1301)

if ch != None:
    if ans == 'y' and args.ccm = 'y':
        ch.random_bytes = (''.join(secrets.choice(string.ascii_uppercase + string.ascii_lowercase + string.digits) for i in range(length - 1)) + '1').encode("utf-8")
    elif ans == 'y' and args.ccm = 'n':
        ch.random_bytes = (''.join(secrets.choice(string.ascii_uppercase + string.ascii_lowercase + string.digits) for i in range(length - 1)) + '0').encode("utf-8")
    elif ans == 'y' and args.ccm != None:
        cc_message = bytes(ccm, 'utf-8')
